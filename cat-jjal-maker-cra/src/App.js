...
const Form = ({updateMainCat}) => {
  ...
  const [errorMessage, setErrorMessatge] = React.useState('');

  function handleInputChange(e) {
    const userValue = e.target.value;
    setErrorMessatge('');
    if(includesHangul(userValue)) {
      setErrorMessatge('한글은 입력할 수 없습니다.');
    }
  ...
  function handleFormSubmit(e) {
    ...
    setErrorMessatge('');
    if(value === '') {
      setErrorMessatge("빈 값으로 만들 수 없습니다.");
      return;
    }
    updateMainCat(value);
  }
...
const App = () => {
  ...
  return (
      ...
      <MainCard img={mainCat} onHeartClick={handleHeartClick} aleadyFavorite={aleadyFavorite} />
      <Faverites favorites={favorites} />
    </div>
  );
}
